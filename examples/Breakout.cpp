...
public:
    ...
    virtual void UserUpdate(Mona::World& world, float timeStep) noexcept
    {
        ...
        if (input.IsKeyPressed(MONA_KEY_A) or input.IsKeyPressed(MONA_KEY_LEFT) or input.IsGamepadButtonPressed(MONA_JOYSTICK_1, MONA_GAMEPAD_BUTTON_DPAD_LEFT) or input.GetGamepadAxisValue(MONA_JOYSTICK_1, MONA_GAMEPAD_AXIS_LEFT_X) < -0.25f) {
            m_transform->Translate(glm::vec3(-m_paddleVelocity * timeStep, 0.0f, 0.0f));
        } else if (input.IsKeyPressed(MONA_KEY_D) or input.IsKeyPressed(MONA_KEY_RIGHT) or input.IsGamepadButtonPressed(MONA_JOYSTICK_1, MONA_GAMEPAD_BUTTON_DPAD_RIGHT) or input.GetGamepadAxisValue(MONA_JOYSTICK_1, MONA_GAMEPAD_AXIS_LEFT_X) > 0.25f) {
            m_transform->Translate(glm::vec3(m_paddleVelocity * timeStep, 0.0f, 0.0f));
        }

        if (input.IsMouseButtonPressed(MONA_MOUSE_BUTTON_1) or input.IsKeyPressed(MONA_KEY_SPACE) or input.IsGamepadButtonPressed(MONA_JOYSTICK_1, MONA_GAMEPAD_BUTTON_A)) {
