...
IKRigController::IKRigController(std::shared_ptr<Skeleton> skeleton, RigData rigData, InnerComponentHandle transformHandle,
    ...
    , m_ikRig(skeleton, rigData, transformHandle)
{
    ...
    glm::vec3 originalFrontVector, AnimationType animationType, float supportFrameDistanceFactor)
{
    ...
    for (int i = 0; i < m_ikRig.getTopology().size(); i++) {
        glblTransforms[i] = glm::identity<glm::mat4>();
    }
    ...
    for (FrameIndex i = 0; i < frameNum; i++) {
        ...
        while (currentIKAnim->getAnimationDuration() <= timeStamp) {
            timeStamp -= 0.000001;
        }
        for (int j = 0; j < currentIKAnim->getJointIndices().size(); j++) {
            ...
            glblTransforms[jIndex] = baseTransform * glmUtils::translationToMat4(animationClip->GetPosition(timeStamp, jIndex, true)) * glmUtils::rotationToMat4(animationClip->GetRotation(timeStamp, jIndex, true)) * glmUtils::scaleToMat4(animationClip->GetScale(timeStamp, jIndex, true));
        }
    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager)
{
    ...
    if (ikAnim.m_onNewFrame) { // se realiza al llegar a un frame de la animacion
        ...
        glm::mat4 nextGlblTransform = glmUtils::translationToMat4(hipTrData->getTargetPositions().evalCurve(targetTimeNext)) * glmUtils::rotationToMat4(glm::angleAxis(m_ikRig.m_rotationAngle + m_ikRig.m_angularSpeed * deltaT, m_ikRig.getUpVector())) * glmUtils::scaleToMat4(glm::vec3(m_ikRig.m_rigScale));
    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager, ComponentManager<SkeletalMeshComponent>& skeletalMeshManager)
{
    ...
    for (AnimationIndex i = 0; i < m_ikRig.m_ikAnimations.size(); i++) {
        IKAnimation& ikAnim = m_ikRig.m_ikAnimations[i];
        if (animController.m_animationClipPtr == ikAnim.m_animationClip || animController.m_crossfadeTarget.GetAnimationClip() == ikAnim.m_animationClip) {
