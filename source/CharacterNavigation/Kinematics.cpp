...
std::function<float(const std::vector<float>&, int, IKData*)> term1PartialDerivativeFunction =
    ...
    for (int c = 0; c < dataPtr->ikChains.size(); c++) {
        ...
        if (ind != -1) {
            ...
            TA = (0 < ind ? dataPtr->forwardModelSpaceTransforms[chain->getJoints()[ind - 1]] : chainBaseTransform) * glmUtils::translationToMat4(TvarTr);

            // matriz que va a la  derecha de la matriz de rotacion de la joint actual en el calculo de la posicion con FK
            TB = glmUtils::scaleToMat4(TvarScl) * (ind < chain->getJoints().size() - 1 ? dataPtr->backwardModelSpaceTransformsPerChain[c][chain->getJoints()[ind + 1]] : glm::identity<glm::mat4>());
...
glm::mat4 ForwardKinematics::JointSpaceTransform(IKAnimation* ikAnim, JointIndex jointIndex, float reproductionTime)
{
    ...
    glm::mat4 jointSpaceTr = glmUtils::translationToMat4(ikAnim->getJointPosition(jointIndex)) * glmUtils::rotationToMat4(glm::angleAxis(rotAngle, rotAxis)) * glmUtils::scaleToMat4(ikAnim->getJointScale(jointIndex));
    return jointSpaceTr;
}
glm::mat4 ForwardKinematics::JointSpaceVariableTransform(IKAnimation* ikAnim, JointIndex jointIndex)
{
    ...
    glm::mat4 jointSpaceTr = glmUtils::translationToMat4(ikAnim->getJointPosition(jointIndex)) * glmUtils::rotationToMat4((*variableJointRotations)[jointIndex].getQuatRotation()) * glmUtils::scaleToMat4(ikAnim->getJointScale(jointIndex));
    return jointSpaceTr;
}
