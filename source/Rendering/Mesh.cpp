...
int orientationTest(const glm::vec3& v1, const glm::vec3& v2, const glm::vec3& testV, float epsilon)
{ // arista de v1 a v2, +1 si el punto esta a arriba, -1 abajo,0 si es colineal, error(-2) si v1 y v2 iguales
    float x1 = v1[0];
    float y1 = v1[1];
    float x2 = v2[0];
    float y2 = v2[1];
    ...
    if (std::abs(testV[0] - v1[0]) <= epsilon && std::abs(testV[1] - v1[1]) <= epsilon || std::abs(testV[0] - v2[0]) <= epsilon && std::abs(testV[1] - v2[1]) <= epsilon) {
        return 0;
    }
