...
{
    return ((gameObject.HasComponent<ComponentTypes>() ? true : (Log::GetLogger()->error("World Error: Component {0} depends on {1}, please add it first", ComponentType::componentName, ComponentTypes::componentName), false))
        && ...);
}
