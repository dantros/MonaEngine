    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager)
{
    ...
    glm::vec2 hipTrEndPredictedXYPos = hipCurrXYPos + hipCurrDir * (glm::distance(glm::vec2(hipPosCurve.evalCurve(currentAnimTime)), glm::vec2(hipPosCurve.getEnd())));
    ...
    // para evitar impresiciones al reorientar la curva por diferencias cercanas a 0 o 180 grados con la direccion objetivo
    if (abs(glm::degrees(m_ikRig->getRotationAngle())) < 20 || 160 < abs(glm::degrees(m_ikRig->getRotationAngle()))) {
        baseCurve.translate(-baseCurve.getStart());
        baseCurve.rotate(glm::angleAxis(glm::degrees(90.0f), m_ikRig->getUpVector()));
    }
    baseCurve.fitEnds(initialPos, finalPos, m_ikRig->getUpVector());
    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager)
{
    ...
    if (ikAnim->isMovementFixed()) {
        ...
        if (!hipTrData->motionInitialized()) {
            ...
            initialPos = glm::vec3(basePoint, calcHipAdjustedHeight(trDataPair, initialTime_rep, ikAnim->getAnimationTime(currentFrame), ikAnim, transformManager, staticMeshManager));
        }
        ...
        while (tCurrExtendedAnim < tInfLimitExtendedAnim) {
            tCurrExtendedAnim += ikAnim->getAnimationDuration();
        }
        while (tSupLimitExtendedAnim < tCurrExtendedAnim) {
            tCurrExtendedAnim -= ikAnim->getAnimationDuration();
        }
        ...
        for (int i = 0; i < 8; i++) {
            if (savedCurve.inTRange(transitionTime2 + avgFrameDuration) && hipPosCurve.inTRange(transitionTime2 + avgFrameDuration)) {
                transitionTime2 += avgFrameDuration;
            }
        }
    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager)
{
    ...
    glm::vec3 floorReferencePoint = glm::vec3(xyReferencePoint, m_environmentData.getTerrainHeight(xyReferencePoint, transformManager, staticMeshManager));
    ...
    ComponentManager<StaticMeshComponent>& staticMeshManager)
{
    ...
    if (baseEETr.isDynamic()) {
        if (m_strideValidationEnabled) {
            ...
            if (oppositeDataAvailable && (baseTrajectoryData->m_fixedTarget && ikAnim->getCurrentFrameIndex() == ikAnim->getFixedMovementFrame() || !baseTrajectoryData->m_fixedTarget)) {
    ...
    for (int i = 0; i < chainNum; i++) {
        ...
        if (ikAnim->getAnimationType() == AnimationType::IDLE) {
            ...
            for (int j = 0; j < frameNum; j++) {
                ikAnim->m_eeTrajectoryData[i].m_supportHeights[j] = globalPositionsPerChain[i][0][2];
            }
        } else {
            ...
            GATHER_POINTS:
                while (baseFrameType == supportFramesPerChain[i][currFrame]) {
                    ikAnim->m_eeTrajectoryData[i].m_supportHeights[currFrame] = baseFrameType ? globalPositionsPerChain[i][currFrame][2] : supportHeight;
