...
public:
    ...
    SubscriptionHandle()
        : m_index(INVALID_EVENT_INDEX)
        , m_generation(0)
        , m_typeIndex(GetEventTypeCount()) {};
...
public:
    ...
    bool IsSubcriptionHandleValid(const SubscriptionHandle& handle)
    {
        ...
        if (index < m_handleEntries.size() && handle.m_generation == m_handleEntries[index].generation && m_handleEntries[index].active == true) {
            return true;
        }
...
private:
    struct HandleEntry {
        HandleEntry(uint32_t i, uint32_t p, uint32_t g)
            : index(i)
            , prevIndex(p)
            , generation(g)
            , active(true)
