...
def get_parser():
    ...
    parser.add_argument('--MONA_generatePrimal', type=bool, default=False)
    parser.add_argument('--MONA_retarget', type=bool, default=False)
...
def get_std_bvh(args=None, character=None, topologyIndex=None):
    if topologyIndex is None:  raise Exception('Must indicate topology')
    if character is None:  raise Exception('Must indicate character')
    if args is None and character is None: raise Exception('Unexpected parameter')
    std_bvh = './datasets/Character_Data/topology{}/std_bvhs/{}.bvh'.format(topologyIndex, character)
    return std_bvh
